...
train_dataset = BDD100KDataset(
    images_dir='D:/BaiduNetdiskDownload/100k/train',
    masks_dir='D:/BaiduNetdiskDownload/masks/train',
    transform=transform
)

val_dataset = BDD100KDataset(
    images_dir='D:/BaiduNetdiskDownload/100k/val',
    masks_dir='D:/BaiduNetdiskDownload/masks/val',
    transform=transform
)
...
def train_model(model, train_loader, val_loader, criterion, optimizer, num_epochs=25,
                model_path='unet_lane_detection.pth', start_epoch=0):
    best_val_loss = 0.266
    ...
    for epoch in range(15, num_epochs):
        ...
        torch.save({
            ...
            'epoch': epoch
        }, f'enet_lane_detection_epoch_{epoch + 1}.pth')

        # Additionally, save the best model
        if val_loss < best_val_loss:
            best_val_loss = val_loss
            torch.save(model.state_dict(), 'best_enet_lane_detection.pth')
...
train_model(model, train_loader, val_loader, criterion, optimizer, num_epochs=20,
            model_path='enet_lane_detection_epoch_15.pth')
